...
double S0(const int K, const int N)
{
    double s=0;
    for (int i=K; i<=N; i++)
        s += (cos(i/2.)+sin(2.*i))/(1+sin(cos(1.*i)));
    return s;
}
double S1(const int K, const int N, const int i)
{
    if(i>N)
        ...
        return (cos(i/2.)+sin(2.*i))/(1+sin(cos(1.*i))) + S1(K, N, i+1);
}
double S2(const int K, const int N, const int i)
{
    if (i<K)
        ...
        return (cos(i/2.)+sin(2.*i))/(1+sin(cos(1.*i))) + S2(K, N, i-1);
}
double S3(const int K, const int N, const int i, double t)
{
    t = t + (cos(i/2.)+sin(2.*i))/(1+sin(cos(1.*i)));
    if (i>=N)
        return t;
    else
        return S3(K, N, i+1, t);
